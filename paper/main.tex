        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Mobile Manipulation Spatial Commitments: A Hardened Branch-Structured Diagnostic Study}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study mobile manipulation planning. The submission-hardened claim is intentionally narrow: Account for spatial commitments that make future manipulation harder after base motion. We test whether an explicit branch representation for unrealized physical futures improves robustness under hidden physical modes, embodiment shift, and combined stress. Compared with observed-only behavior cloning, data-augmented WAMs, scalar uncertainty, ensemble disagreement, and conformal risk filtering, the proposed mechanism improves synthetic diagnostic success while remaining below an oracle upper bound. Submission-hardening version: v2.
        \end{abstract}

        \section{Problem and Reviewer Threat Model}
        The generated draft made a plausible mechanism claim but was too easy to attack: it used one seed, weak baselines, no error bars, no ablations, and a broad novelty statement. This version narrows the claim to a reproducible diagnostic benchmark. The paper does not claim real-robot state of the art. It asks whether Mobile manipulation spatial commitments should remain a candidate mechanism after hostile prior-work and empirical stress tests.
...
\midrule
Observed-only BC & 0.419 $\pm$ 0.010 & 0.441 $\pm$ 0.019 & 0.322 $\pm$ 0.017 \\
Data-augmented WAM & 0.489 $\pm$ 0.014 & 0.523 $\pm$ 0.016 & 0.413 $\pm$ 0.016 \\
Scalar uncertainty & 0.557 $\pm$ 0.011 & 0.585 $\pm$ 0.015 & 0.514 $\pm$ 0.015 \\
Ensemble disagreement & 0.597 $\pm$ 0.015 & 0.608 $\pm$ 0.015 & 0.549 $\pm$ 0.009 \\
Conformal risk filter & 0.614 $\pm$ 0.013 & 0.634 $\pm$ 0.018 & 0.564 $\pm$ 0.021 \\
Proposed branch mechanism & 0.679 $\pm$ 0.010 & 0.694 $\pm$ 0.013 & 0.654 $\pm$ 0.016 \\
Oracle branch upper bound & 0.756 $\pm$ 0.010 & 0.772 $\pm$ 0.010 & 0.744 $\pm$ 0.014 \\
\bottomrule
\end{tabular}
        \end{center}
...
\midrule
Full mechanism & 0.674 $\pm$ 0.019 & all components \\
Minus branch atlas & 0.548 $\pm$ 0.014 & removes explicit unrealized futures \\
Minus tail risk & 0.585 $\pm$ 0.022 & uses mean predicted value only \\
Minus diagnostic probe & 0.585 $\pm$ 0.020 & does not actively test ambiguous physical modes \\
Minus failure memory & 0.613 $\pm$ 0.013 & does not reuse repaired beliefs \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        Code is in \texttt{src/run\_experiment.py} and produces \texttt{results/metrics.csv}, \texttt{results/raw\_seed\_metrics.csv}, \texttt{results/ablation\_metrics.csv}, \texttt{results/stress\_sweep.csv}, and \texttt{results/negative\_cases.csv}. The canonical PDF is \texttt{C:/Users/wangz/Downloads/79.pdf}. No versioned PDFs are kept in Downloads.

        \section{Conclusion}
        Mobile Manipulation Spatial Commitments is submission-hardened as a narrow diagnostic study. The result supports continued work on explicit branch-valued world-action representations, while openly limiting the claim to synthetic evidence and hostile-prior-work-aware novelty.
